...
pub async fn get_job_list(
    ...
    match jobs {
        ...
        Err(e) => {
            ApiResponse::<String>::error(format!("获取定时任务列表失败: {}", e).as_str()).respond()
        }
    }
}
    ...
    match result {
        Ok(Some(job_model)) => {
            ...
            match active_job.update(db).await {
                Ok(_) => {
                    ApiResponse::<String>::success_with_msg("更新定时任务状态成功", None).respond()
                }
                Err(e) => {
        ...
        Ok(None) => ApiResponse::<String>::error("定时任务不存在").respond(),
        Err(e) => ApiResponse::<String>::error(format!("查询定时任务失败: {}", e).as_str()).respond(),
    }
}
...
pub async fn run_job_once(
    ...
    ApiResponse::<String>::success_with_msg("执行定时任务成功", None).respond()
}
    ...
    match schedule_job::Entity::delete_by_id(job_id).exec(db).await {
        Ok(result) => {
            if result.rows_affected > 0 {
                ApiResponse::<String>::success_with_msg("删除定时任务成功", None).respond()
            } else {
                ApiResponse::<String>::error("定时任务不存在").respond()
            }
        }
        Err(e) => ApiResponse::<String>::error(format!("删除定时任务失败: {}", e).as_str()).respond(),
    }
}
...
pub async fn add_job(
    ...
    match new_job.insert(db).await {
        Ok(_) => ApiResponse::<String>::success_with_msg("添加定时任务成功", None).respond(),
        Err(e) => ApiResponse::<String>::error(format!("添加定时任务失败: {}", e).as_str()).respond(),
    }
}
...
pub async fn edit_job(
    ...
    if job.job_id.is_none() {
        return ApiResponse::<String>::error("定时任务ID不能为空").respond();
    }
    ...
    match result {
        Ok(Some(job_model)) => {
            ...
            match active_job.update(db).await {
                ...
                Err(e) => ApiResponse::<String>::error(format!("更新定时任务失败: {}", e).as_str()).respond(),
            }
        }
        Ok(None) => ApiResponse::<String>::error("定时任务不存在").respond(),
        Err(e) => ApiResponse::<String>::error(format!("查询定时任务失败: {}", e).as_str()).respond(),
...
pub async fn get_job_log_list(
    ...
    match logs {
        ...
        Err(e) => ApiResponse::<String>::error(format!("获取定时任务日志列表失败: {}", e).as_str()).respond(),
    }
}
        ...
        .await
    {
        Ok(result) => {
            if result.rows_affected > 0 {
                ...
                ApiResponse::<String>::error("定时任务日志不存在").respond()
            }
        }
        Err(e) => ApiResponse::<String>::error(format!("删除定时任务日志失败: {}", e).as_str()).respond(),
    }
}
